...
class ChainlinkBtcUsdRtds:
    ...
    def open_price_at_boundary(
        self, boundary_unix_s: int, fallback_before: bool = True
        ...
        **不检查 lag**：不管这条 tick 晚到多少秒都返回。
        ...
        fallback_before: 若无边界后的 tick，是否回退到边界前 RTDS_OPEN_FALLBACK_MAX_MS 内的最新 tick。
        """
        target_ms = int(boundary_unix_s) * 1000
        max_before = int(float(os.environ.get("RTDS_OPEN_FALLBACK_MAX_MS", "30000")))
        with self._lock:
            best_after: Optional[Tuple[int, float]] = None
            best_before: Optional[Tuple[int, float]] = None
            for ts_ms, v in self._ticks:
                if ts_ms >= target_ms:
                    if best_after is None or ts_ms < best_after[0]:
                        best_after = (ts_ms, v)
                elif fallback_before and target_ms - ts_ms <= max_before:
                    if best_before is None or ts_ms > best_before[0]:
                        best_before = (ts_ms, v)
            return best_after if best_after is not None else best_before
